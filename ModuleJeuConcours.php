			...
			if(isset($_POST['FORM_SUBMIT']))
			{
  			...
  			switch($this->Session->get('participationjeux_'.$this->id))
  			{
  				...
  				case 'second':
					...
					$this->Template->content = '<div style="float:right"><img src="tl_files/images/jeux-uploads/etape2.jpg" alt="Etape 2/2"></div><div class="clear"></div><br /><form action="'.$_action.'" id="xkn_form_jeu_end" method="post" enctype="application/x-www-form-urlencoded">
					...
					$this->Template->content .= '<div class="info">En validant votre participation, vous certifiez avoir lu et accept&eacute; le r&egrave;glement de jeu dans son int&eacute;gralit&eacute;.</div><br /><div style="float:right"><div class="submit_container"><input type="submit" class="submit" id="submit_jeu_end" value="Valider votre participation" /></div></div></form><div class="clear"></div>';
			      ...
			      $objJeux = $this->Database->prepare("SELECT * FROM tl_xkn_jeu_concours WHERE id = ? ORDER BY id")->execute($this->xkn_id_game);  
						...
						$_form->User = $this->User;
						$this->Template->content = '<div style="float:right"><img src="tl_files/images/jeux-uploads/etape1.jpg" alt="Etape 1/2"></div><div class="clear"></div><br />'.$_form->parse();
								$GLOBALS['TL_HEAD'][] = '<link media="screen" type="text/css" href="/plugins/formcheck/theme/red/formcheck.css" rel="stylesheet" />
						';
						$GLOBALS['TL_HEAD'][] = '<script type="text/javascript" src="/plugins/formcheck/lang/fr.js"> </script>';
						$GLOBALS['TL_HEAD'][] = '<script type="text/javascript" src="/plugins/formcheck/formcheck.js"> </script>';
